...
** \details   It is up to the user to configure the desired CAN clock source. It can be
**            HSE, PLL1Q, or PLL2Q. To meet the clock tolerance requirement of CAN 2.0B,
**            an external crystal oscillator (HSE) is recommended. A multiple of 8 MHz
**            gives the best support for most commonly used CAN baudrates, so that is
**            recommended.
** \return    none.
**
****************************************************************************************/
static void BootComCanInit(void)
{
...
#endif
  unsigned long       fifoElementDataSize = FDCAN_DATA_BYTES_8;
   ...
   * which should be done first, otherwise the call to HAL_RCCEx_GetPeriphCLKFreq() to
...
#if (BOOT_COM_CAN_FD_ENABLE > 0)
  fifoElementDataSize = FDCAN_DATA_BYTES_64;
  ...
  canHandle.Init.RxFifo0ElmtSize = fifoElementDataSize;
  canHandle.Init.RxFifo1ElmtsNbr = 0;
  canHandle.Init.RxFifo1ElmtSize = fifoElementDataSize;
  canHandle.Init.RxBuffersNbr = 0;
  canHandle.Init.RxBufferSize = fifoElementDataSize;
  ...
  canHandle.Init.TxElmtSize = fifoElementDataSize;
