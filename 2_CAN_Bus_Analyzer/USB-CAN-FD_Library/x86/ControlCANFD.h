...
CHAR	str_hw_Type[40];
USHORT	Reserved[4]; 	
} VCI_BOARD_INFO,*PVCI_BOARD_INFO; // A total of 63 bytes

//2. Define the data type of CAN information frames. Used now.
Typedef struct _VCI_CN_OBJ {//32 bytes.

UINT	ID;
UINT	TimeStamp;// Retain not using 32-bit, four byte.
BYTE	TimeFlag;// No need to keep it
BYTE	SendType;// No need to keep it
BYTE	RemoteFlag;// Is it a remote frame
BYTE	ExternFlag;// Is it an extended frame
BYTE	DataLen;
BYTE	Data[8];
...
DWORD	AccMask;
DWORD	Reserved;
UCHAR	Filter;// 0,1 receives all frames. 2 standard frame filtering and 3 extended frame filtering.
UCHAR	Timing0; 	
UCHAR	Timing1; 	
UCHAR	Mode; // Mode, 0 represents normal mode, 1 represents listening only mode, 2 represents self-test mode
